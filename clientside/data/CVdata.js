//-----------------------------------------------------------------------------------------------------------
var data_CVtitle = {
    "moi_photo": "",
    "moi_name": ""
};

var data_CVinfo = {
    "info_raisonsociale": [],
    "info_adresse": [],
    "info_email": [],
    "info_phonenum": [],
};

var data_CVexperience = {
    "exp_title": "",
    "exp_content": []
};

var data_CVskillshead = {
    "mskills_title": ""
};
var data_CVskillsentries = [
    {
        "mskills_f1desc": [],
        "mskills_f1level": [],
        "mskills_f1text": []
    },{
        "mskills_f2desc": [],
        "mskills_f2level": [],
        "mskills_f2text": []
    },{
        "mskills_f3desc": [],
        "mskills_f3level": [],
        "mskills_f3text": []
    },{
        "mskills_f4desc": [],
        "mskills_f4level": [],
        "mskills_f4text": []
    }
];

var data_CVlanghead = {
    "lang_title": ""
};
var data_CVlangentries = [
    {
        "lang_f1desc": [],
        "lang_f1level": [],
        "lang_f1text": []
    },{
        "lang_f2desc": [],
        "lang_f2level": [],
        "lang_f2text": []
    },{
        "lang_f3desc": [],
        "lang_f3level": [],
        "lang_f3text": []
    }
];

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot0 = {
    "boulotentry0date": [],
    "boulotentry0boite": [],
    "boulotentry0desc": []
};

var data_CVboulot01 = {
    "boulotentry0item": [],
    "boulotentry0title": [],
    "boulotentry0content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot1 = {
    "boulotentry1date": [],
    "boulotentry1boite": [],
    "boulotentry1desc": []
};

var data_CVboulot11 = {
    "boulotentry1item": [],
    "boulotentry1title": [],
    "boulotentry1content": []
};

var data_CVboulot12 = {
    "boulotentry2item": [],
    "boulotentry2title": [],
    "boulotentry2content": []
};

var data_CVboulot13 = {
    "boulotentry3item": [],
    "boulotentry3title": [],
    "boulotentry3content": []
};

var data_CVboulot14 = {
    "boulotentry4item": [],
    "boulotentry4title": [],
    "boulotentry4content": []
};

var data_CVboulot15 = {
    "boulotentry5item": [],
    "boulotentry5title": [],
    "boulotentry5content": []
};

var data_CVboulot16 = {
    "boulotentry6item": [],
    "boulotentry6title": [],
    "boulotentry6content": [],
    "boulotentry7title": [],
    "boulotentry7content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot2 = {
    "boulotentry8date": [],
    "boulotentry8boite": [],
    "boulotentry8desc": []
};

var data_CVboulot21 = {
    "boulotentry8item": [],
    "boulotentry8title": [],
    "boulotentry8content": []
};

var data_CVboulot22 = {
    "boulotentry9item": [],
    "boulotentry9title": [],
    "boulotentry9content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot3 = {
    "boulotentry10date": [],
    "boulotentry10boite": [],
    "boulotentry10desc": []
};

var data_CVboulot31 = {
    "boulotentry10item": [],
    "boulotentry10title": [],
    "boulotentry10content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot4 = {
    "boulotentry11date": [],
    "boulotentry11boite": [],
    "boulotentry11desc": []
};

var data_CVboulot41 = {
    "boulotentry11item": [],
    "boulotentry11title": [],
    "boulotentry11content": []
};

var data_CVboulot42 = {
    "boulotentry12item": [],
    "boulotentry12title": [],
    "boulotentry12content": []
};

var data_CVboulot43 = {
    "boulotentry13item": [],
    "boulotentry13title": [],
    "boulotentry13content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot5 = {
    "boulotentry14date": [],
    "boulotentry14boite": [],
    "boulotentry14desc": []
};

var data_CVboulot51 = {
    "boulotentry14item": [],
    "boulotentry14title": [],
    "boulotentry14content": []
};

var data_CVboulot52 = {
    "boulotentry15item": [],
    "boulotentry15title": [],
    "boulotentry15content": []
};

//-----------------------------------------------------------------------------------------------------------
// BOULOT ENTRY
var data_CVboulot6 = {
    "boulotentry16date": [],
    "boulotentry16boite": [],
    "boulotentry16desc": []
};

var data_CVboulot61 = {
    "boulotentry16item": [],
    "boulotentry16title": [],
    "boulotentry16content": []
};

//-----------------------------------------------------------------------------------------------------------
// BILDUNG
var data_CVbildung1 = {
    "edu_f1date": [],
    "edu_f1boite": null,
    "edu_f1desc": null
};

var data_CVbildung11 = {
    "edu_f11item": [],
    "edu_f11title": [],
    "edu_f11content": []
};

var data_CVbildung12 = {
    "edu_f12item": [],
    "edu_f12title": [],
    "edu_f12content": []
};

var data_CVbildung13 = {
    "edu_f13item": [],
    "edu_f13title": [],
    "edu_f13content": []
};

// LOISIRS
var data_CVhobby1= {
    "loisirs_f1date": [],
    "loisirs_f1boite": null,
    "loisirs_f1desc": null,
};

var data_CVhobby11 = {
    "loisirs_f11item": [],
    "loisirs_f11title": [],
    "loisirs_f11content": []
};

var data_CVhobby12 = {
    "loisirs_f12item": [],
    "loisirs_f12title": [],
    "loisirs_f12content": []
};

var data_CVhobby13 = {
    "loisirs_f13item": [],
    "loisirs_f13title": [],
    "loisirs_f13content": []
};

//-----------------------------------------------------------------------------------------------------------
// STRUCTS
var data_CVstructleft = [
    [ data_CVtitle ],
    [ data_CVinfo ],
    [ data_CVexperience ],
    [ data_CVskillshead, data_CVskillsentries ],
    [ data_CVlanghead, data_CVlangentries ]
];

var data_CVstructright = [
    {   boite: data_CVboulot0,
        boulots: [ data_CVboulot01 ],
    },
    {   boite: data_CVboulot1,
        boulots: [ data_CVboulot11, data_CVboulot12, data_CVboulot13, data_CVboulot14, data_CVboulot15, data_CVboulot16 ],
    },
    {   boite: data_CVboulot2,
        boulots: [ data_CVboulot21, data_CVboulot22 ],
    },
    {   boite: data_CVboulot3,
        boulots: [ data_CVboulot31 ],
    },
    {
        boite: data_CVboulot4,
        boulots: [ data_CVboulot41, data_CVboulot42, data_CVboulot43 ],
    },
    {   boite: data_CVboulot5,
        boulots: [ data_CVboulot51, data_CVboulot52 ],
    },
    {   boite: data_CVboulot6,
        boulots: [ data_CVboulot61 ],
    },
    {   boite: data_CVbildung1,
        boulots: [ data_CVbildung11, data_CVbildung12, data_CVbildung13 ],
    },
    {   boite: data_CVhobby1,
        boulots: [ data_CVhobby11, data_CVhobby12, data_CVhobby13 ],
    }
];

//-----------------------------------------------------------------------------------------------------------
// MAP
var data_CVmap = [
    //---------------------------------------
    {   side: "left",
        data: "data_CVtitle",
        cbk: (CV, d) => { CV.addtitle(document, d); }
    },
    {   side: "left",
        data: "data_CVinfo",
        cbk: (CV, d) => { CV.addinfo(document, d); }
    },
    {   side: "left",
        data: "data_CVexperience",
        cbk: (CV, d) => { CV.addexperience(document, d); }
    },
    {   side: "left",
        data: [ "data_CVskillshead", "data_CVskillsentries" ],
        cbk: (CV, d) => { CV.adddetails(document, d); }
    },
    {   side: "left",
        data: [ "data_CVlanghead", "data_CVlangentries" ],
        cbk: (CV, d) => { CV.adddetails(document, d); }
    },
    //---------------------------------------
    {   side: "right",
        boite: "data_CVboulot0",
        boulots: [ "data_CVboulot01" ],
        progress: [ "boulotentry0wait", "boulotentry0progress", "boulotentry0bar" ],
    },
    {   side: "right",
        boite: "data_CVboulot1",
        boulots: [ "data_CVboulot11", "data_CVboulot12", "data_CVboulot13", "data_CVboulot14", "data_CVboulot15", "data_CVboulot16" ],
        progress: [ "boulotentry1wait", "boulotentry1progress", "boulotentry1bar" ],
    },
    {   side: "right",
        boite:  "data_CVboulot2",
        boulots: [ "data_CVboulot21", "data_CVboulot22" ],
        progress: [ "boulotentry8wait", "boulotentry8progress", "boulotentry8bar" ],
    },
    {   side: "right",
        boite:  "data_CVboulot3",
        boulots: [ "data_CVboulot31" ],
        progress: [ "boulotentry10wait", "boulotentry10progress", "boulotentry10bar" ],
    },
    {   side: "right",
        boite: "data_CVboulot4",
        boulots: [ "data_CVboulot41", "data_CVboulot42", "data_CVboulot43" ],
        progress: [ "boulotentry11wait", "boulotentry11progress", "boulotentry11bar" ],
    },
    {   side: "right",
        boite: "data_CVboulot5",
        boulots: [ "data_CVboulot51", "data_CVboulot52" ],
        progress: [ "boulotentry14wait", "boulotentry14progress", "boulotentry14bar" ],
    },
    {   side: "right",
        boite: "data_CVboulot6",
        boulots: [ "data_CVboulot61" ],
        progress: [ "boulotentry16wait", "boulotentry16progress", "boulotentry16bar" ],
    },
    {   side: "right",
        boite: "data_CVbildung1",
        boulots: [ "data_CVbildung11", "data_CVbildung12", "data_CVbildung13" ],
        progress: [ "edu_f1wait", "edu_f1progress", "edu_f1bar" ],
    },
    {   side: "right",
        boite: "data_CVhobby1",
        boulots: [ "data_CVhobby11", "data_CVhobby12", "data_CVhobby13" ],
        progress: [ "loisirs_f1wait", "loisirs_f1progress", "loisirs_f1bar" ],
    }
];

//-----------------------------------------------------------------------------------------------------------
// PRINT
var data_CVprintindexes = [
//  item[0], item[1]

    [ "Training", -1 ],
    [ "Diploma", -1 ],
    [ "Activity", -1 ],
    [ "Sport", -1 ],
    [ "Martial arts", -1 ],
    [ "Running", -1 ],
    [ "Triathlon", -1 ],
    [ "Music", -1 ],
    [ "Guitar", -1 ],
    [ "Bass guitar", -1 ],
    [ "Interests", -1 ],
    [ "Various", -1 ],

    [ "<b>Description</b>", -1 ],
    [ "<b>Environment</b>", -1 ],
    [ "<b>Keypoints</b>", -1 ],
    [ "<b>Methodology</b>", -1 ]

];